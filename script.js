...
// Gera links externos seguros.
function safeLink(url, label) {
  if (!url || url === "#") {
    return `<span class="tag">${label}: adicionar link</span>`;
  }
...
// Renderiza os dados do portfólio.
function renderData() {
  ...
  $("#aboutContent").innerHTML = `
    <div class="hero-bio">
      ...
      <div class="avatar">
        <img
          src="tatsuya_profile.png"
          alt="Imagem de perfil de B. Tatsuya"
        >
      </div>
      ...
      <div>
        ...
        <p>
          <b>WHAT_I_LIKE_TO_SOLVE.txt</b>
          <br>
          ${portfolioData.professionalInterest}
        </p>
  ...
  $("#projectsContent").innerHTML = `
    ...
    ${portfolioData.projects.map(p => `
      <article class="project-card">
        ...
        <p>
          ${safeLink(p.github, "GitHub Repository")}
        </p>
  ...
  $("#skillsContent").innerHTML = `
    ...
    <div class="panel">
      ...
      <b>
        CREATIVE_ARCHIVE.dat
      </b>

      <p>
        ${portfolioData.personality.creativeBackground}
      </p>
      ...
      <p>
        ...
        <a
          href="https://open.spotify.com/intl-pt/artist/2fkMzB82VphBTaEHOazlkj"
          target="_blank"
          rel="noopener"
          title="Abrir Agatha Sepulta no Spotify"
        >
          <b>
            ${portfolioData.personality.creativeProject}
          </b>
        </a>

      </p>

      ${portfolioData.personality.curiosities.map(x => `
        <span class="tag">
          ${x}
        </span>
      `).join("")}
  ...
  $("#competenciesContent").innerHTML = `
    ...
    >
      ...
      ${portfolioData.complementaryTraining.map(x => `
        <span class="tag">
          ${x}
        </span>
      `).join("")}
    ...
    >
      ...
      <b>
        CORE_COMPETENCIES.sys
      </b>

      <br><br>

      ${portfolioData.competencies.map(x => `
        <span class="tag">
          ${x}
        </span>
      `).join("")}
    ...
    >
      ...
      <b>
        CULTURAL_PROJECTS.log
      </b>

      <br><br>

      ${portfolioData.culturalProjects.map(p => `
        <p>
          <b>${p.name}</b>
          // ${p.role}
          // ${p.context}
    ...
    <div class="panel">
      ...
      <b>
        OTHER_EXPERIENCE.dat
      </b>

      <br><br>

      ${portfolioData.otherExperience.map(x => `
        <span class="tag">
          ${x}
        </span>
      `).join("")}
  ...
  $("#contactContent").innerHTML = `
    ...
    <div class="panel">
      ...
      <p style="color:var(--muted)">
        CURRENT_STATUS //
        aberta a oportunidades, projetos,
        freelance e networking.
      </p>
...
// Estado dos efeitos sonoros.
const soundState = {
  ctx: null,
  muted:
    localStorage.getItem(
      "tatsuyadesire97_sfx_muted"
    ) === "1"
};
...
function syncAmbientMute() {
  ...
  const now =
    soundState.ctx.currentTime;

  ambientState.master.gain
...
function setupSoundFX() {
  ...
  document.addEventListener(
    ...
    e => {
      ...
      if (
        e.target.closest(
          "button, .file, .desktop-icon, .start-item, .occult-mark"
        )
      ) {
...
// Converte o nome da janela para ID.
function windowId(name) {
  return `win-${name}`;
}
...
// Maximiza ou restaura uma janela.
...
// Configura controles das janelas.
...
// Configura os elementos que abrem janelas.
...
function runShutdownSequence() {
  ...
  text.textContent =
    "Closing open memories.";

  status.textContent =
    "SYSTEM // preparing shutdown";


  clearTimeout(
    runShutdownSequence.t1
  );

  clearTimeout(
    runShutdownSequence.t2
  );

  clearTimeout(
    runShutdownSequence.t3
  );
  ...
  runShutdownSequence.t1 =
    setTimeout(
      () => {
        ...
        text.textContent =
          "Writing last known state to disk.";
  ...
  runShutdownSequence.t2 =
    setTimeout(
      () => {
        ...
        text.textContent =
          "TatsuyaDesire/97 has entered dream state.";
  ...
  runShutdownSequence.t3 =
    setTimeout(
      () => {
        ...
        title.textContent =
          "It is now safe to leave this website.";
...
function runCommand(input) {
  ...
  const commands = {
    ...
    help:
      "help · about · experience · skills · projects · contact · clear · whoami · date · theme · ghost",
  ...
  if (cmd === "ghost") {
    ...
    toast(
      enabled
        ? "WARNING // spectral interference detected"
        : "ghost.exe detached // signal stable"
    );
  ...
  if (cmd === "theme") {
    ...
    appendTerminal(
      "  dreamgaze  // DREAMGAZE_PDX (default)"
    );

    appendTerminal(
      "  spectral   // SPECTRAL_PURPLE_97"
    );

    appendTerminal(
      "  moonmilk   // pale blue / silver haze"
    );

    appendTerminal(
      "  rosefog    // dusty pink / nocturnal bloom"
    );

    appendTerminal(
      "  liminalsea // drowned teal / analog dream"
    );

    appendTerminal(
      "Use: theme <name>  // ex: theme spectral"
    );
  ...
  if (
    ...
    if (
      ...
      appendTerminal(
        `Theme loaded: ${themeName.toUpperCase()}`
      );

      toast(
        `theme://${themeName} // dream palette mounted`
      );
      ...
      appendTerminal(
        "Type 'theme' to list available themes."
      );
    }
  ...
  appendTerminal(
    commands[cmd] ??
    `Bad command or file name: ${cmd || "(empty)"}`
  );
}
// Classes dos temas alternativos.
const themeClasses = [
  "theme-spectral",
  "theme-moonmilk",
  "theme-rosefog",
...
function setupTerminal() {
  ...
  appendTerminal(
    "TatsuyaDesire/97 Terminal v0.97"
  );
  ...
  mark.addEventListener(
    ...
    () => {
      ...
      if (
        ...
        toast(
          "archive://agatha_sepulta // signal detected"
        );
      }
  ...
  document.addEventListener(
    ...
    e => {
      ...
      if (
        ...
        toast(
          "GOTHIC MODE // no church bells found, only reverb."
        );
  ...
  $("#ghostFile")
    .addEventListener(
      ...
      () => {
        ...
        toast(
          "WARNING // definitely_not_a_ghost.exe // visual anomaly detected",
          5200
        );
        ...
        setTimeout(
          () => {
            ...
            toast(
              "ghost.exe vanished // signal returned to normal"
            );
...
// Controla o carregamento opcional do Spotify.
function setupSpotifyConsent() {
  ...
  const consent =
    localStorage.getItem(
      "tatsuyadesire97_spotify_consent"
    );


  // Cria o iframe somente depois da autorização.
  function loadSpotify() {
    ...
    iframe.src =
      portfolioData.spotify.embedUrl;
  ...
  // Mostra o estado em que o Spotify foi recusado.
  function renderRejectedState() {
    ...
    consentBox.innerHTML = `
      <p>
        O player do Spotify não foi carregado.
      </p>
  ...
  // Se já aceitou anteriormente, carrega.
  if (
    consent === "accepted"
  ) {
  ...
  // Se recusou anteriormente, mantém bloqueado.
  if (
    consent === "rejected"
  ) {
...
// Ajusta o tamanho do Spotify.
function setupSpotifyResponsiveScale() {

  const bezel =
    document.querySelector(
      "#win-music .spotify-bezel"
    );
...
// IMPORTANTE:
// agora o Spotify passa pelo consentimento.
setupSpotifyConsent();
